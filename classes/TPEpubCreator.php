...
class TPEpubCreator
{
    ...
    public function CreateEPUB() {
        ...
        foreach( $this->pages as $key => $value ) {
            ...
            // OPF
            $opf .= '<item id="' . $page . '" href="html/' . $page . '.xhtml" media-type="application/xhtml+xml" />' . "\r\n";
            ...
            $ncx .= '</navLabel>' . "\r\n";
            $ncx .= '<content src="html/' . $page . '.xhtml"/>' . "\r\n";
            $ncx .= '</navPoint>' . "\r\n";
        ...
        if ( ! empty( $this->images ) ) {
            foreach( $this->images as $image_key => $image_value ) {
                ...
                $new_image  = $this->temp_folder . '/OEBPS/html/docimages/';
                ...
                if ( ! empty( $image_value['cover'] ) ) {
                    $opf .= '<item id="cover" href="html/cover.xhtml" media-type="application/xhtml+xml" />' . "\r\n";
                ...
                $opf .= '" href="docimages/' . basename( $new_image );
        ...
        foreach( $this->pages as $key => $value ) {
            ...
            foreach( $this->images as $check_image_key => $check_images ) {
                $value['content'] = str_replace ( 
                    $check_images['path'],
                    'docimages/' . basename( $this->new_images[$check_image_key]['path'] ),
                    $value['content']
                );
            }
            
            // Create the file
            $this->CreateFile( $this->temp_folder . '/OEBPS/html/' . $page, $value['content'] );
        }
        
        // If there's a cover, create its page
        if ( ! empty( $this->cover_img )  ) {
            $cover_page  = $page_content;
            $cover_page .= '<img class="cover-image" width="600" height="800" src="docimages/' . $this->cover_img . '" />' . "\r\n";
            $cover_page .= '</body></html>';
            $this->CreateFile( $this->temp_folder . '/OEBPS/html/cover.xhtml', $cover_page );
        }
    ...
    private function OpenOPF() {
        ...
        $this->opf[] = '<dc:identifier id="BookID" opf:scheme="identifier">' . $this->identifier . '</dc:identifier>' . "\r\n";
    ...
    private function OpenNCX() {
        ...
        $this->ncx[] = '<meta name="dtb:uuid" content="' . $this->uuid . '"/>' . "\r\n";
        $this->ncx[] = '<head>' . "\r\n";
    ...
    private function CreateFolders() {
        ...
        mkdir( $this->temp_folder . '/OEBPS/html', 0777 );
        mkdir( $this->temp_folder . '/OEBPS/html/docimages', 0777 );
        ...
        $this->CreateFile( $this->temp_folder . '/OEBPS/html/css.css', $this->css);
    ...
    private function CreateZip () {
        ...
        $folders = array(
            ...
            $this->temp_folder . '/OEBPS/html',
            $this->temp_folder . '/OEBPS/html/docimages',
            $this->temp_folder, 
        );
        ...
        rmdir( $this->temp_folder . '/OEBPS/html/docimages' );
        rmdir( $this->temp_folder . '/OEBPS/html' );
